...
for name, (_, label, color) in RUNS.items():
    ...
    axes[0, 0].annotate(
        f"{best_ce:.3f} at epoch {best}", (best, best_ce),
        xytext=(15, 29 if name == "teacher" else -26),
...
axes[1, 1].set_ylim(40, 54)
axes[1, 1].set_xlim(0, 750)
axes[1, 1].legend(frameon=False, loc="lower right", fontsize=9)
for ax in axes.flat:
    ax.set_xlabel("Training epoch")
    ax.grid(axis="y", color="#dddddd", lw=0.6)
fig.suptitle("Condition C run comparison", fontsize=19, weight="bold", x=0.07, ha="left")
fig.text(0.07, 0.929, "W&B snapshot: 8 September 2026 | matched comparison at epoch 640 | unsmoothed validation values",
